...
if __name__ == "__main__":
    # other examples: https://github.com/hunterjm/futgui/blob/master/frames/playersearch.py
    _=lambda x: x
    ...
    class MyApplication(object):
        ...
        def __init__(self, master):
            ...
            self.etv = EditableTreeview(self.mainwindow)

            self.etv["columns"]=("type","url")
            self.etv.column("type", anchor='center', width=60)
            self.etv.column("url")
            self.etv.column('#0', anchor='w', width=30)
            self.etv.heading('#0', text=_("Header"), anchor='w')
            self.etv.heading("type", text=_("Type"))
            self.etv.heading("url", text=_("URL"))
            self.etv.bind('<<TreeviewInplaceEdit>>', self.on_row_edit)
            self.etv.bind('<<TreeviewCellEdited>>', self.on_cell_changed)
            self.etv.pack()
            self.allow_edit = tk.BooleanVar()
